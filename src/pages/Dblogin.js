...
import { dbloginRedux } from '../redux/dbSlice';
...
const Dblogin = () => {
  ...
  const handleSubmit = async(e)=> {
    ...
    if (email && password) {
      const fetchData = await fetch(`${process.env.REACT_APP_SERVER_DOMIN}/dblogin`,{
        method : "POST",
        headers : {
  ...
  return (
    ...
    <div className="w-full max-w-sm bg-white m-auto flex  flex-col p-4 rounded-[20px] mt-0">
      ...
      <p className="text-left text-sm mt-2">
        Don't have account ?{" "}
        <Link to={"/Dbregister"} className="text-blue-900 underline">
          SignUp
        </Link>
      </p>
    </div>
